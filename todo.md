...
## BUG REPORTADO - Erro 404 ao criar orçamento (19/01/2026 - Tarde)

- [ ] Erro 404 ao criar orçamento a partir do modal "Criar de Fichas"
  - Sintoma: Clica em "Criar Orçamento", modal fecha, mas recebe erro 404
  - Causa: Investigar rota de redirecionamento ou criação do orçamento
  - Prioridade: CRÍTICA - Bloqueia fluxo de criação de orçamento
  - Status: EM INVESTIGAÇÃO

## MELHORIA SOLICITADA - Filtro por cliente no modal de fichas (19/01/2026 - Tarde)

- [ ] Adicionar filtro por cliente no modal SelecionarFichasModal
  - Motivo: Com muitas fichas, fica difícil encontrar as do cliente desejado
  - Solução: Dropdown para filtrar fichas por cliente
  - Benefício: Usuário seleciona cliente → vê apenas fichas daquele cliente
  - Implementação: Usar coluna 'cliente' da tabela fichas_custo
  - Status: PLANEJADO


## BUGS CORRIGIDOS E MELHORIAS - SESSÃO 19/01/2026 (TARDE)

### ✅ BUG CORRIGIDO - Erro 404 ao criar orçamento
- [x] Corrigir rota de navegação de /orcamentos para /orcamento
- [x] Testar fluxo completo de criação com nova rota
- [x] Status: RESOLVIDO - Orçamentos criando com sucesso

### ✅ MELHORIA IMPLEMENTADA - Filtro por cliente no modal
- [x] Adicionar dropdown para filtrar fichas por cliente
- [x] Extrair clientes únicos das fichas
- [x] Filtrar fichas em tempo real ao selecionar cliente
- [x] Testar seleção com filtro ativo
- [x] Status: IMPLEMENTADO E TESTADO - Funciona perfeitamente

### ✅ CORREÇÃO - Cálculo de custo em fichas
- [x] Adicionar cálculo de custo total na função getFichasCustoByUser
- [x] Custo = modelagem + piloto + corte + costura + acabamento + embalagem
- [x] Status: RESOLVIDO - Custos calculados corretamente no modal
